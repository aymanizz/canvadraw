...
export type Modifier =
    'none' | 'ctrl' | 'shift' | 'alt'
    | 'ctrl+shift' | 'shift+alt' | 'ctrl+alt'
    | 'ctr+shift+alt'

const modifierOf = (event: KeyboardEvent) => {
    const modifiers = []
    if (event.ctrlKey)
        modifiers.push('ctrl')
    if (event.shiftKey)
    ...
    if (event.altKey)
        modifiers.push('alt')
    return <Modifier>modifiers.join('+') || 'none'
}
...
export default class ShortcutsRegistry<T> {
    ...
    put(code: string, modifier: Modifier, handler: KeyboardEventHandler<T>) {
